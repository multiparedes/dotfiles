...
require('lazy').setup({
  ...
  -- NOTE: mini.nvim (mini.ai + mini.statusline + minimal start screen) lives in
...
vim.keymap.set('n', "<leader>b", function()
  -- On non-file buffers (e.g. mini.starter) skip reveal to avoid the
  -- "File not in cwd. Change cwd to ...?" prompt.
  if vim.bo.buftype ~= "" or vim.fn.filereadable(vim.api.nvim_buf_get_name(0)) == 0 then
    vim.cmd("Neotree toggle reveal=false")
  else
    vim.cmd("Neotree toggle")
  end
end, {nowait = false, noremap = true, silent = true, desc = "Neotree toggle"})
...
-- Python indentation management (detect/toggle/reformat) — defines <leader>T, <leader>vf, :PyIndent
dofile(vim.fn.stdpath("config") .. "/python_indent.lua")
